...
//    private ListView lv;
    ...
    private LinearLayout mVoiceChatLayout;
//    private ShSwitchView mVoiceOpenSwitch,mBluetoothSwitch;
    private ShSwitchView mVoiceOpenSwitch;
...
//        lv = (ListView) findViewById(R.id.lv);
        dl = (DragLayout) findViewById(R.id.drag_layout);
        ivBottom = (ImageView) findViewById(R.id.user_ico);
        mListView = (CustomerListView) findViewById(R.id.listView_left_side);
        mVoiceChatLayout=(LinearLayout)findViewById(R.id.voice_chat_layout);
        mVoiceOpenSwitch = (ShSwitchView) findViewById(R.id.voice_chat_switch);
    ...
    public void initListener() {
        mListView.setAdapter(quickAdapter = new QuickAdapter<ItemBean>(this, R.layout.item_left_layout, ItemDataUtils.getItemBeans()) {
            @Override
...
//                Log.d("switchButton0", state ? "开":"关");
//                if(state == true) {
//                    Log.d("switchButton1", "开");
//                    Toast.makeText(MainActivity.this,  "开", Toast.LENGTH_SHORT).show();
//                }
//                else{
//                    Log.d("switchButton2", "关");
...
//        });
    ...
    private void SwitchChanged() {
        ...
        mVoiceOpenSwitch.setOnSwitchStateChangeListener(new ShSwitchView.OnSwitchStateChangeListener() {
            @Override
            //
            public void onSwitchStateChange(boolean isOn) {
                if (isOn == true) {
                    Log.d("mVoiceOpenSwitch1", "开");
                    startService(new Intent(MainActivity.this, VoiceChatService.class));//跳转服务界面
                } else {
                    Log.d("mVoiceOpenSwitch2", "关");
                    stopService(new Intent(MainActivity.this, VoiceChatService.class));
                }
            }
        });
    }
...
//        if (isVoiceChatServiceOpening(MainActivity.this)) {
        if (isServiceRunning(MainActivity.this,"VoiceChatService")) {
            mVoiceOpenSwitch.setOn(true);   //打开
            Log.e("mVoiceOpenSwitch11：","open");
        } else {

            mVoiceOpenSwitch.setOn(false);  //关闭
            Log.e("mVoiceOpenSwitch12：","off");
        }
    }
    //    static  判断服务是否在运行
